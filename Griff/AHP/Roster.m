classdef Roster
    ...
    methods
        ...
        function [tasksOut, workersOut] = makeCast(obj, size, offset)
            if offset>0
                castTasks = size;
        ...
        function [taskScores, workerScores] = scorePairs(obj, pairs, ranks, offset)
            ...
            taskSat = 0;
            realMatches = 0;
            taskStep = 1/castWorkers;
            workerStep = 1/castTasks;
            debug = "SCORING TASKS";
            for x = 1:castTasks
                pick = pairs(x, 1);
                rank  = ranks(x, 1);
                if pairs(x, 1)>castWorkers
                    ...
                    debug = "OPTION 2";
                    taskSat     = taskSat + ((castWorkers - ranks(x, 1) + 1) * taskStep);
                end
            end
            ...
            for x = 1:castWorkers
                if pairs(x, 2)>castTasks
                    ...
                    debug = "OPTION 2";
                    workerSat   = workerSat + ((castTasks - ranks(x, 2) + 1) * workerStep);
